...
for i, alpha in enumerate(learning_rates):
    ...
    for j, temperature in enumerate(temperatures):
        ...
        if j == 0 and i == 1:
            ax[i][j].set_ylabel(f'Avg Reward over steps and all {n_runs} runs', fontsize=12)
        if j == 2:
            text = ax[i][j].text(6000,.25,f"Alpha: {alpha}", size=12, rotation=270)
        if j == 1 and i == 2:
        ...
        for model in model_name:
            ...
            ax[i][j].fill_between(list(range(len(runs_values_avg))), runs_values_avg-runs_values_std, runs_values_avg+runs_values_std,alpha=0.2,
...
class qlearning:
    ...
    def gen_table(self, env, bins,initial_table=None):
        if initial_table is None:
            self.q = np.random.uniform(low=-0.001, high=0.001, size=(bins, bins, bins, bins, 2))
        else:
            self.q = initial_table
